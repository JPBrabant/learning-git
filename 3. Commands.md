...
## `log`
  ...
  - `git log`                    : Show the whole commit history
      ...
      - `(-n 10|--max-count 10)` : Last 10 commit
...
Remove files from the index, or from the working tree **AND** the index. The next commit will show the file deleted, that tell git to stop tracking the file. It the same thing as doing a normal linux `rm` and then staging the deletion with `git add`. 
...
Though not required, it's a good idea to begin the commit message with a single short (no more than 50 characters) line summarizing the change, followed by a blank line and then a more thorough description. 
  ...
  - `git commit`   : Commit the current state of the index. An editor will open to allow you to write a commit message. 
      - `--amend` : Overwrite the last commit with the current state of the index. Add `--no-edit` if you don't want to change the commit message.
...
New command in 2.54 to modify history easier than git rebase interactive. 

  - `git history reword <commit>` : Rewrite the commit message of the specified commit.

  - `git history split <commit>`  : Interactively split up `<commit>` into two commits by choosing hunks.
...
## `push`
  ...
  - `git push [<branch>]`            : Push `<branch>` or the current branch to its remote tracking branch.
      - `--all|--branches`           : Push all branches.
      - `--tags`                     : Push tags (annotated and lightweight).
      - `(-u|--set-upstream) origin` : Push a branch that you've never pushed before.
